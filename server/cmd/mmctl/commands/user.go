...
Arguments:
  from_auth:
    ...
    Supported options: email, ldap, office365, saml.
...
func migrateAuthToSamlCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	...
	if fromAuth == "" || (fromAuth != "email" && fromAuth != "ldap" && fromAuth != "office365") {
...
func migrateAuthToLdapCmdF(c client.Client, cmd *cobra.Command, userArgs []string) error {
	fromAuth := userArgs[0]
	if fromAuth == "" || (fromAuth != "email" && fromAuth != "saml" && fromAuth != "office365") { // nolint:goconst
		return errors.New("invalid from_auth argument")
	}
