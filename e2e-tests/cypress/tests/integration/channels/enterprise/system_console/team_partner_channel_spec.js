...
const getButtonId = (permission) => {
    return 'partners-partner_' + permission + '-checkbox';
};

const disableAllPartnerPermissions = () => {
...
const enableAllPartnerPermissions = () => {
    permissions.forEach((permission) => {
        cy.findByTestId(getButtonId(permission)).then((btn) => {
            if (!btn.hasClass('checked')) {
                btn.click();
...
const verifyAllPartnerPermissions = (selected) => {
    permissions.forEach((permission) => {
        if (selected) {
...
describe('Team Scheme Partner Permissions Test', () => {
    before(() => {
        // * Check if server has license
        cy.apiRequireLicense();
    });

    it('MM- - Enable and Disable all partner permission', () => {
        ...
        verifyAllPartnerPermissions(true);

        // # Uncheck all the boxes currently checked
        disableAllPartnerPermissions();
        ...
        verifyAllPartnerPermissions(false);
