...
// Group: @channels @not_cloud @enterprise @partner_account @mfa
...
describe('Partner Accounts', () => {
    ...
    it('MM-T1390 Enforce Partner MFA when MFA is enabled and enforced', () => {
        ...
        // # Check if user is allowed to enforce MFA for Partner accounts.
        cy.findByTestId('PartnerAccountsSettings.EnforceMultifactorAuthenticationtrue').check();
        ...
        // # From the main page, invite a Partner user and click on the Join Team in the email sent to the partner user.
        cy.visit(`/${testTeam.name}/channels/town-square`);

        // # Open team menu, click Invite People, then invite partner
        cy.uiOpenTeamMenu('Invite people');
        cy.findByTestId('invitePartnerLink').click();

        // # Type partner user e-mail address.
        ...
        cy.getRecentEmail({username, email}).then((data) => {
            ...
            expect(subject).to.contain(`${sysadmin.username} invited you to join the team ${testTeam.display_name} as a partner`);
        ...
        // * When MFA is enforced for Partner Access, partner user should be forced to configure MFA while creating an account.
