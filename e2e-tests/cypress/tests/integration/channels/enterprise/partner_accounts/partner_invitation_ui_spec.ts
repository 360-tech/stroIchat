...
describe('Partner Account - Partner User Invitation Flow', () => {
    ...
    it('MM-T4451 Verify UI Elements of Partner User Invitation Flow', () => {
        // # Open team menu and click 'Invite People'
        cy.uiOpenTeamMenu('Invite people');

        // * Verify Invite Partner link
        ...
        cy.findByTestId('invitationModal').within(() => {
            cy.get('h1').should('have.text', `Invite partners to ${testTeam.display_name}`);
        });

        // * Verify Invite Partners button is disabled by default
        ...
        cy.get('.users-emails-input__menu').
            ...
            eq(0).should('contain', `Invite ${email} as a partner`).click();
    ...
    it('MM-T1386 Verify when different feature settings are disabled', () => {
        ...
        changePartnerFeatureSettings(false, true);
        ...
        // * Verify if Invite Members modal is displayed when partner account feature is disabled
        ...
        changePartnerFeatureSettings(true, false);
        ...
        // * Verify Invite Partners button is disabled
        cy.findByTestId('inviteButton').should('be.disabled');
    });

    it('MM-T4449 Invite Partner via Email containing upper case letters', () => {
        // # Reset Partner Feature settings
        changePartnerFeatureSettings();
    ...
    it('MM-T1414 Add Partner from Add New Members dialog', () => {
        // # Demote the user from member to partner
        cy.apiDemoteUserToPartner(newUser.id);
        ...
        cy.findByTestId('invitationModal').within(() => {
            ...
            cy.get('div.invitation-modal-confirm--not-sent').should('be.visible').within(() => {
                ...
                cy.get('.reason').should('have.text', 'Contact your admin to make this partner a full member.');
    ...
    it('MM-T1415 Check invite more button available on both successful and failed invites', () => {
        ...
        // * Verify the content and message in next screen
        cy.findByText('This person is already a member of the workspace. Invite them as a member instead of a partner.').should('be.visible');
    ...
    it('hides the copy link button when inviting partners', () => {
        // # Open team menu and click 'Invite People'
        cy.uiOpenTeamMenu('Invite people');
